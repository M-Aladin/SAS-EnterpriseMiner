...
_LP0 = _LP0 + (    0.25590757464835 * _TEMP);

***  Effect: CleanserProducts ;
_TEMP = CleanserProducts ;
_LP0 = _LP0 + (    0.07400254079358 * _TEMP);

***  Effect: DayCareProducts ;
_TEMP = DayCareProducts ;
_LP0 = _LP0 + (    0.14270744830453 * _TEMP);

***  Effect: FacebookLike ;
_TEMP = 1;
_LP0 = _LP0 + (   -0.35336913779254) * _TEMP * _2_0;

***  Effect: MobileApplications ;
_TEMP = 1;
_LP0 = _LP0 + (   -0.40812989999405) * _TEMP * _4_0;

***  Effect: NightRepairProducts ;
_TEMP = NightRepairProducts ;
_LP0 = _LP0 + (    0.39255830640348 * _TEMP);

***  Effect: PreviousCampaign ;
_TEMP = 1;
_LP0 = _LP0 + (   -0.35649905599258) * _TEMP * _5_0;

***  Effect: REP_Age ;
_TEMP = REP_Age ;
_LP0 = _LP0 + (   -0.08992256142797 * _TEMP);

***  Effect: REP_Gender ;
_TEMP = 1;
_LP0 = _LP0 + (    1.08480215664264) * _TEMP * _8_0;
_LP0 = _LP0 + (   -0.31566464466076) * _TEMP * _8_1;

*** Naive Posterior Probabilities;
drop _MAXP _IY _P0 _P1;
_TEMP =    -0.39236485517339 + _LP0;
if (_TEMP < 0) then do;
   _TEMP = exp(_TEMP);
   _P0 = _TEMP / (1 + _TEMP);
end;

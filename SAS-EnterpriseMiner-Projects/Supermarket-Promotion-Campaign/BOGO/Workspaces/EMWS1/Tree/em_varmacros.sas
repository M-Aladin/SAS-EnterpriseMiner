...
%let EM_NUM_VARS = 19;
   %macro EM_VARS;
    AdvertisementZone AffluenceGrade Age CampaignViewedEmail CleanserProducts
   ClusterGroup DayCareProducts FacebookLike Gender LotionProducts LoyaltyClass
   LoyaltyTime MobileApplications NightRepairProducts PreviousCampaign Region
   TargetBuy TimeSinceLastPurchase TotalSpend
   %mend EM_VARS;
%let EM_NUM_CLASS = 10;
   %macro EM_CLASS;
    AdvertisementZone CampaignViewedEmail ClusterGroup FacebookLike Gender
   LoyaltyClass MobileApplications PreviousCampaign Region TargetBuy
   %mend EM_CLASS;
%let EM_NUM_INTERVAL = 9;
   %macro EM_INTERVAL;
    AffluenceGrade Age CleanserProducts DayCareProducts LotionProducts LoyaltyTime
...
%let EM_NUM_NOMINAL_INPUT = 5;
   %macro EM_NOMINAL_INPUT;
    AdvertisementZone ClusterGroup Gender LoyaltyClass Region
